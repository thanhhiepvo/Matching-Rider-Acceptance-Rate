"""Optuna tuning cho MLP (PyTorch) — mở rộng từ LightGBM V2 (tune_lightgbm.py) để so sánh 4
model class công bằng hơn ở compare_models.py (trước đó XGBoost/CatBoost/MLP dùng tham số đặt
tay, chỉ LightGBM được tune).

Mỗi trial train ÍT epoch hơn train_mlp.py gốc (`--epochs-per-trial` mặc định 15, patience 3
thay vì 40/5) — NN tốn thời gian hơn hẳn cây quyết định mỗi trial, cần giảm ngân sách/trial để
tổng thời gian tuning không quá dài. Early-stop + chọn trial theo ROC-AUC trên `valid`
(split.py) — KHÔNG đụng `test`/`calib`.

    python3 Baseline/tune_mlp.py [--n-trials 20] [--epochs-per-trial 15]
"""
from __future__ import annotations

import argparse
import json
import os

import mlflow
import numpy as np
import optuna
import pandas as pd
import torch
import torch.nn as nn
from sklearn.metrics import average_precision_score, roc_auc_score
from torch.utils.data import DataLoader

from evaluation import evaluate
from features import FEATURES, PRE_CATEGORICAL, build_features, load_raw
from mlp_common import (
    TabularDataset,
    TabularMLP,
    apply_categorical_encoders,
    encode_categoricals,
    fit_numeric_scaler,
    transform_numeric,
)
from split import align_categories, time_split
from train import ART, EXPERIMENT_NAME, MLFLOW_DB_PATH, RAW, ROOT

optuna.logging.set_verbosity(optuna.logging.WARNING)

OPTUNA_DB_PATH = os.path.join(ROOT, "optuna.db")
STUDY_NAME = "mlp_tuning"
NUMERIC_FEATURES = [f for f in FEATURES if f not in PRE_CATEGORICAL]
DEVICE = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
SEED = 42
HIDDEN_CHOICES = {"small": (64, 32), "medium": (128, 64), "large": (256, 128, 64)}

torch.manual_seed(SEED)
np.random.seed(SEED)


def log_metrics(prefix: str, m: dict):
    for k, v in m.items():
        if k == "confusion_matrix":
            cm = v
            mlflow.log_metrics({
                f"{prefix}_cm_tp": cm["tp"], f"{prefix}_cm_fp": cm["fp"],
                f"{prefix}_cm_fn": cm["fn"], f"{prefix}_cm_tn": cm["tn"],
            })
        else:
            mlflow.log_metric(f"{prefix}_{k}", v)


def train_one(params: dict, X_tr_num, X_tr_cat, y_tr, X_va_num_t, X_va_cat_t, y_va,
              cat_cardinalities, epochs: int, patience: int):
    """Train với early-stopping (theo dõi AUC trên X_va_*) — dùng cho GIAI ĐOẠN TUNING
    (X_va_* = `valid`). KHÔNG dùng hàm này cho final retrain (xem `train_fixed_epochs`)."""
    model = TabularMLP(n_numeric=len(NUMERIC_FEATURES), cat_cardinalities=cat_cardinalities,
                        embed_dim=params["embed_dim"], hidden=HIDDEN_CHOICES[params["hidden"]],
                        dropout=params["dropout"]).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=params["lr"], weight_decay=params["weight_decay"])
    loss_fn = nn.BCEWithLogitsLoss()

    ds = TabularDataset(X_tr_num, X_tr_cat, y_tr)
    loader = DataLoader(ds, batch_size=params["batch_size"], shuffle=True, drop_last=True)

    best_auc, best_state, best_epoch, no_improve = -1.0, None, 0, 0
    for epoch in range(1, epochs + 1):
        model.train()
        for xb_num, xb_cat, yb in loader:
            xb_num, xb_cat, yb = xb_num.to(DEVICE), xb_cat.to(DEVICE), yb.to(DEVICE)
            opt.zero_grad()
            loss = loss_fn(model(xb_num, xb_cat), yb)
            loss.backward()
            opt.step()

        model.eval()
        with torch.no_grad():
            p_va = torch.sigmoid(model(X_va_num_t, X_va_cat_t)).cpu().numpy()
        auc = average_precision_score(1 - np.asarray(y_va), 1 - p_va)  # Cancel PR-AUC, không phải ROC-AUC
        if auc > best_auc:
            best_auc, best_epoch = auc, epoch
            best_state = {k: v.clone() for k, v in model.state_dict().items()}
            no_improve = 0
        else:
            no_improve += 1
            if no_improve >= patience:
                break
    model.load_state_dict(best_state)
    return model, best_auc, best_epoch


def train_fixed_epochs(params: dict, X_tr_num, X_tr_cat, y_tr, cat_cardinalities, n_epochs: int):
    """Train ĐÚNG n_epochs cố định, KHÔNG early-stop, KHÔNG cần eval set — dùng cho final
    retrain trên train+valid gộp (giống tune_lightgbm.py dùng lại best_iteration cố định thay
    vì early-stop lần 2, tránh phải đụng thêm `test` hay `calib` chỉ để chọn số epoch)."""
    model = TabularMLP(n_numeric=len(NUMERIC_FEATURES), cat_cardinalities=cat_cardinalities,
                        embed_dim=params["embed_dim"], hidden=HIDDEN_CHOICES[params["hidden"]],
                        dropout=params["dropout"]).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=params["lr"], weight_decay=params["weight_decay"])
    loss_fn = nn.BCEWithLogitsLoss()
    ds = TabularDataset(X_tr_num, X_tr_cat, y_tr)
    loader = DataLoader(ds, batch_size=params["batch_size"], shuffle=True, drop_last=True)
    for _ in range(n_epochs):
        model.train()
        for xb_num, xb_cat, yb in loader:
            xb_num, xb_cat, yb = xb_num.to(DEVICE), xb_cat.to(DEVICE), yb.to(DEVICE)
            opt.zero_grad()
            loss = loss_fn(model(xb_num, xb_cat), yb)
            loss.backward()
            opt.step()
    model.eval()
    return model


def make_objective(X_tr_num, X_tr_cat, y_tr, X_va_num_t, X_va_cat_t, y_va,
                    cat_cardinalities, epochs: int, patience: int):
    def objective(trial: optuna.Trial) -> float:
        params = {
            "embed_dim": trial.suggest_int("embed_dim", 2, 12),
            "hidden": trial.suggest_categorical("hidden", list(HIDDEN_CHOICES)),
            "dropout": trial.suggest_float("dropout", 0.0, 0.5),
            "lr": trial.suggest_float("lr", 1e-4, 1e-2, log=True),
            "weight_decay": trial.suggest_float("weight_decay", 1e-6, 1e-3, log=True),
            "batch_size": trial.suggest_categorical("batch_size", [256, 512, 1024]),
        }
        _, best_auc, best_epoch = train_one(params, X_tr_num, X_tr_cat, y_tr, X_va_num_t, X_va_cat_t, y_va,
                                             cat_cardinalities, epochs, patience)
        trial.set_user_attr("best_epoch", best_epoch)
        return best_auc
    return objective


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--n-trials", type=int, default=20)
    ap.add_argument("--epochs-per-trial", type=int, default=15)
    ap.add_argument("--patience", type=int, default=3)
    args = ap.parse_args()

    os.makedirs(ART, exist_ok=True)
    mlflow.set_tracking_uri(f"sqlite:///{MLFLOW_DB_PATH}")
    mlflow.set_experiment(EXPERIMENT_NAME)

    with mlflow.start_run(run_name="mlp-v2-tuned"):
        mlflow.set_tags({"phase": "post_dispatch_only", "model": "mlp_pytorch_v2_optuna"})
        mlflow.log_params({"n_trials": args.n_trials, "epochs_per_trial": args.epochs_per_trial})

        print("1 · Nạp data + chia train/valid/test THEO THỜI GIAN (split.py)")
        orders, customer_daily = load_raw(RAW)
        df, X, y = build_features(orders, customer_daily, verbose=False)
        split = time_split(df, X, y, post_only=True)
        X_tr, y_tr = split.train
        X_va, y_va = align_categories(X_tr, split.valid[0]), split.valid[1]
        X_te, y_te = align_categories(X_tr, split.test[0]), split.test[1]
        print(f"  train {len(X_tr):,} · valid {len(X_va):,} · test {len(X_te):,}")

        print("2 · Tiền xử lý (log1p + scale numeric, encode categorical) — fit trên train")
        scaler = fit_numeric_scaler(X_tr, NUMERIC_FEATURES)
        X_tr_num = transform_numeric(X_tr, scaler)
        X_va_num = transform_numeric(X_va, scaler)
        tr_codes, va_codes, encoders = encode_categoricals(X_tr[PRE_CATEGORICAL], X_va[PRE_CATEGORICAL])
        X_tr_cat = np.stack([tr_codes[c] for c in PRE_CATEGORICAL], axis=1)
        X_va_cat = np.stack([va_codes[c] for c in PRE_CATEGORICAL], axis=1)
        cat_cardinalities = [len(encoders[c][0]) for c in PRE_CATEGORICAL]
        X_va_num_t = torch.tensor(X_va_num, dtype=torch.float32).to(DEVICE)
        X_va_cat_t = torch.tensor(X_va_cat, dtype=torch.long).to(DEVICE)

        print(f"3 · Optuna search ({args.n_trials} trial × {args.epochs_per_trial} epoch/trial, "
              f"tối ưu Cancel PR-AUC trên valid)")
        storage = f"sqlite:///{OPTUNA_DB_PATH}"
        try:
            optuna.delete_study(study_name=STUDY_NAME, storage=storage)
        except KeyError:
            pass
        study = optuna.create_study(study_name=STUDY_NAME, storage=storage,
                                     direction="maximize", sampler=optuna.samplers.TPESampler(seed=42))
        study.optimize(make_objective(X_tr_num, X_tr_cat, y_tr.values, X_va_num_t, X_va_cat_t, y_va,
                                       cat_cardinalities, args.epochs_per_trial, args.patience),
                        n_trials=args.n_trials, show_progress_bar=False)
        best_trial = study.best_trial
        best_epoch = best_trial.user_attrs["best_epoch"]
        print(f"  best trial #{best_trial.number}: valid Cancel PR-AUC {best_trial.value:.4f}, best_epoch={best_epoch}")
        mlflow.log_params({f"best_{k}": v for k, v in best_trial.params.items()})
        mlflow.log_metric("best_valid_pr_auc_cancel", best_trial.value)
        mlflow.log_param("best_epoch_on_valid", best_epoch)

        trials_path = os.path.join(ART, "optuna_trials_mlp.csv")
        study.trials_dataframe().to_csv(trials_path, index=False)
        mlflow.log_artifact(trials_path)

        print(f"4 · Retrain final trên train+valid gộp — ĐÚNG best_epoch={best_epoch} tìm được ở bước 3, "
              f"KHÔNG early-stop lại (giống tune_lightgbm.py, tránh đụng test/calib chỉ để chọn epoch)")
        X_trva, y_trva = split.train_valid
        scaler_f = fit_numeric_scaler(X_trva, NUMERIC_FEATURES)
        X_trva_num = transform_numeric(X_trva, scaler_f)
        X_te_final = align_categories(X_trva, split.test[0])
        tr_codes_f, te_codes_f, encoders_f = encode_categoricals(X_trva[PRE_CATEGORICAL], X_te_final[PRE_CATEGORICAL])
        X_trva_cat = np.stack([tr_codes_f[c] for c in PRE_CATEGORICAL], axis=1)
        X_te_cat = np.stack([te_codes_f[c] for c in PRE_CATEGORICAL], axis=1)
        cat_cardinalities_f = [len(encoders_f[c][0]) for c in PRE_CATEGORICAL]
        X_te_num = transform_numeric(X_te_final, scaler_f)
        X_te_num_t = torch.tensor(X_te_num, dtype=torch.float32).to(DEVICE)
        X_te_cat_t = torch.tensor(X_te_cat, dtype=torch.long).to(DEVICE)

        model = train_fixed_epochs(best_trial.params, X_trva_num, X_trva_cat, y_trva.values,
                                    cat_cardinalities_f, n_epochs=best_epoch)

        print("5 · Đánh giá trên test")
        model.eval()
        with torch.no_grad():
            p_te = torch.sigmoid(model(X_te_num_t, X_te_cat_t)).cpu().numpy()
        metrics = {"test": evaluate(split.test[1], p_te, "test")}
        log_metrics("test", metrics["test"])

        print("6 · Hệ thống đầy đủ (rule + model) — như train_mlp.py")
        is_test_full = df.order_date >= pd.Timestamp("2026-07-13")
        X_te_all = align_categories(X_trva, X[is_test_full].copy())
        y_te_all = y[is_test_full]
        post_te_all = df[is_test_full].is_post_dispatch.astype(int).values == 1
        X_all_num = transform_numeric(X_te_all, scaler_f)
        X_all_cat = apply_categorical_encoders(X_te_all[PRE_CATEGORICAL], encoders_f)
        with torch.no_grad():
            p_all = torch.sigmoid(model(
                torch.tensor(X_all_num, dtype=torch.float32).to(DEVICE),
                torch.tensor(X_all_cat, dtype=torch.long).to(DEVICE))).cpu().numpy()
        p_te_all = np.where(post_te_all, p_all, 0.0)
        metrics["system_full"] = evaluate(y_te_all, p_te_all, "system-full")
        log_metrics("system_full", metrics["system_full"])

        print("7 · Lưu artifact")
        import joblib
        model_path = os.path.join(ART, "mlp_v2_tuned.pt")
        torch.save(model.state_dict(), model_path)
        preprocess_path = os.path.join(ART, "mlp_v2_tuned_preprocess.pkl")
        joblib.dump({"scaler": scaler_f, "encoders": encoders_f, "cat_cardinalities": cat_cardinalities_f,
                     "numeric_features": NUMERIC_FEATURES, "categorical_features": PRE_CATEGORICAL,
                     "best_params": best_trial.params}, preprocess_path)
        metrics["best_params"] = best_trial.params
        metrics["features"] = FEATURES
        metrics_path = os.path.join(ART, "metrics_mlp_v2.json")
        json.dump(metrics, open(metrics_path, "w"), indent=2, ensure_ascii=False)
        mlflow.log_artifact(metrics_path)
        mlflow.log_artifact(model_path)
        mlflow.log_artifact(preprocess_path)

        print(f"\n✓ -> {ART}")
        print(f"✓ MLflow run: {mlflow.active_run().info.run_id}")


if __name__ == "__main__":
    main()
